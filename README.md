...
执行用时 : 92 ms, 在Remove Duplicates from Sorted Array的Python3提交中击败了59.56% 的用户  
...
执行用时 : 56 ms, 在Remove Element的Python3提交中击败了34.07% 的用户  
...
执行用时 : 52 ms, 在Implement strStr()的Python3提交中击败了45.90%的用户  
...
执行用时 : 52 ms, 在Search in Rotated Sorted Array的Python3提交中击败了58.86% 的用户  
...
class Solution:
    def searchRange(self, nums: List[int], target: int) -> List[int]:
        if target not in nums:
            return [-1,-1]
        temp = nums[::-1]
        return [nums.index(target),len(nums)-temp.index(target)-1]
```

执行用时 : 56 ms, 在Find First and Last Position of Element in Sorted Array的Python3提交中击败了44.33% 的用户  
...
执行用时 : 76 ms, 在Valid Sudoku的Python3提交中击败了72.13% 的用户  
...
执行用时 : 64 ms, 在Valid Sudoku的Python3提交中击败了98.57% 的用户  
...
测试通过，提交超时，放到pycharm里测试通过，但是需要修改递归深度，我的电脑默认递归深度是2148，经过测试，该测试的递归深度是8416，而提交时报错的递归深度是27056，所以需要重置递归深度：
...
执行用时 : 60 ms, 在Count and Say的Python3提交中击败了29.56% 的用户  
